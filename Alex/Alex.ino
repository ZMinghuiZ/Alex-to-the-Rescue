...
//Colour sensor
#define S0 7  //(1 << 7) // pin7     
#define S1 8  //(1 << 0) // pin8
#define S2 9  //(1 << 1) // pin9
#define S3 12  //(1 << 4) // pin12
#define OUT 13  //(1 << 5) // pin13

int colour_data;
int RBG[3] = {0, 0, 0};        
int Black[3] = {330, 250, 330}; //baseline reading
int White[3] = {25, 20, 25};  //baseline reading
...
void setupEINT()
{
  ...
  EICRA |= 0b1010;     // INT0/1 both falling
  EIMSK |= 0b11;            // enable INT0/1
}
...
void proxy_alarm()
{
  ...
  if (distance <= 10.0) // distance within 10 cm alarm is triggered
  {
    ringalarm();
    delay(1000);
    stopalarm();
...
void identify()
{
   ...
   //Colour = 0 RED  Colour = 1 GREEN  Colour = 2 Not victim
